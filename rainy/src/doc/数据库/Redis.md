...
![img_8.png](Redis/md/img_8.png)

## 缓存更新策略：
内存淘汰，超时剔除(低一致性)，主动更新（高一致性）

![img_9.png](Redis/md/img_9.png)
...
> ![img_10.png](Redis/md/img_10.png)

### 缓存穿透：
_请求客户端数据中，缓存里面和数据库里面数据都不存在，请求都打到数据库上面了_
>解决办法：布隆过滤器（内存占用少），缓存空对象或者null（额外内存消耗，可能造成短期不一致）
> ![img_11.png](Redis/md/img_11.png)
>主动添加 对ID的复杂度

### 缓存雪崩

> ![img_12.png](Redis/md/img_12.png)
> 很多key同时失效
...
> 2：逻辑过期TTL=-1
>
> ![img_13.png](Redis/md/img_13.png)
>
> ![img_14.png](Redis/md/img_14.png)

选择一致性还是可用性

> 全局唯一 ID生成器
![img_15.png](Redis/md/img_15.png)

UUID：没有满足特性 ，RedisId：数字类型，snowflake：算法，维护机器ID

### 多线程下高并发超卖问题是怎么发生的

> ![img_17.png](Redis/md/img_17.png)
>
> ![img_18.png](Redis/md/img_18.png)
>
...
![img_16.png](Redis/md/img_16.png)
...
![img_19.png](Redis/md/img_19.png)

![img_20.png](Redis/md/img_20.png)
...
![img_21.png](Redis/md/img_21.png)

极端情况线程出现并发安全问题。

![img_22.png](Redis/md/img_22.png)

解决办法：释放锁的时候需要进行检查，看自己的锁标识是否和之前获取的一样。

![img_23.png](Redis/md/img_23.png)

![img_24.png](Redis/md/img_24.png)
...
![img_25.png](Redis/md/img_25.png)

![img_26.png](Redis/md/img_26.png)

## Redisson问题(呵呵，这是一个把分布式锁实现好的开源框架)

![img_27.png](Redis/md/img_27.png)

![img_28.png](Redis/md/img_28.png)

![img_29.png](Redis/md/img_29.png)

## redisson 分布式锁原理：

![img_30.png](Redis/md/img_30.png)
...
![img_31.png](Redis/md/img_31.png)

### 利用Redis来优化秒杀
使用Set类型，解决唯一性问题。lua脚本来解决锁释放和事务提交的原子性问题

![img_32.png](Redis/md/img_32.png)

将下单信息添加到阻塞队列里面

![img_33.png](Redis/md/img_33.png)
...
> 消费者：从消息队列中获取消息并处理
>
> ![img_34.png](Redis/md/img_34.png)
>
...
![img_35.png](Redis/md/img_35.png)

![img_36.png](Redis/md/img_36.png)

![img_37.png](Redis/md/img_37.png)
...
![img.png](Redis/md/消息队列.png)

### 三种消息队列List PubSub Stream 解决方案的区别
![img_1.png](Redis/md/消息队列模式.png)
...
> 拉模式（读扩散）：缺点是延迟高
>
> ![img_2.png](Redis/md/推模式.png)
>
> 推模式
>
> ![img_3.png](Redis/md/拉模式.png)
>
> 推拉结合：
> ![img.png](Redis/md/推拉结合模式.png)
>
> 收件箱使用redis来实现，查询redis查询收件箱数据时需要实现分页查询，list，soretedset，数据的角标会发生变换，按照传统的分页方案将会出现错误
>
> ![img_1.png](Redis/md/feed分页问题.png)
>
> ![img_2.png](Redis/md/滚动分页.png)
>
